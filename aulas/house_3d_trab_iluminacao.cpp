...
void Camera::move_forward(float distance) {
    float dx = centerX - obsX;
    float dz = centerZ - obsZ;
...
void Camera::rotate_right(float angle) {
    ...
    float dx = centerX - obsX;
    float dz = centerZ - obsZ;
    float newDx = dx * cos(radians) - dz * sin(radians);
    float newDz = dx * sin(radians) + dz * cos(radians);
    centerX = obsX + newDx;
    centerZ = obsZ + newDz;
}
...
void House::set_angle_x(float angle) { angle_x = angle; }
void House::set_angle_y(float angle) { angle_y = angle; }
void House::set_angle_z(float angle) { angle_z = angle; }
...
void draw_houses() {
    ...
    GLfloat posicaoLuz[4]={100.0,1.0,1.0,1.0};
    glLightfv(GL_LIGHT0, GL_POSITION, posicaoLuz);
...
void handle_mouse_click(int button, int state, int x, int y) {
    ...
    if (button == GLUT_LEFT_BUTTON) current_house = &house1;
    if (button == GLUT_RIGHT_BUTTON) current_house = &house2;
}
